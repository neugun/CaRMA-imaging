function strDir_ImgWarp = Cluster_Scripts_Reg_SyN_2p_par_useRef(stPaths_G, strDir_ImgSeq, strImgFn_prefix, idx_S, idx_E, idx_Ref, bClusterOE)
% Generate job shell script for nonlinear image registration with reference
% image generated before and submit the job to the computer cluster. 
...
for nFrame = idx_S:idx_E
    ...
    strANTSCmd = [strANTSCmdFn ' --float 1 -v ' strVerb ' -d 2 -r [' strImgFn_Ref ',' strImgFn ',0] \\\n'...
        ...
        '-t Translation[0.1] -c [300x300x300x300,5.e-6,10] -s 3x2x1x0vox -f 8x4x2x1 \\\n' ...
        '-m MI[' strImgFn_Ref  ',' strImgFn ',1,32] \\\n'...
        '-t Affine[0.1] -c [300x300x300x300,5.e-6,10] -s 3x2x1x0vox -f 8x4x2x1 \\\n' ...
