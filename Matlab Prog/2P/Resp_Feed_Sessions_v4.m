strFn_Resp = 'Z:\Imaging\2P345\20171030\ANM372321\Reg_Cat\Avg3_nbg\Resp_Corrected.mat';
strFn_States='Z:\Imaging\2P345\20171030\ANM372321\States.xlsx';
load(strFn_Resp);
[strPath,strFn] = fileparts(strFn_Resp);
%%nNeuronCount = size(matResp,1);

AvgFrm = 3;
vtEvents = [156,157,181];
idxTN_Rg =[50 100];
nTrialCount = 7;
szBin= 5;
%%
...
%%
idx_rg_c = 1:450; % do functional clustering according to the temporal responses within this range
dColorThresh = 0.4;
%%
strPath_Results = [strPath '\Results_' num2str(idx_rg_c(1)) '-' num2str(idx_rg_c(end)) '_Sel'];
if(~exist(strPath_Results,'dir'))
    mkdir(strPath_Results);
end
clResultTypes = {'Raw','NormRow_01','NormROC'};
%%
vtSel = setxor(1:101,[17 19 29 73 75 92 93])';
...
for nType = 1:length(clResultTypes)
    ...
    if(nType ==3)
        ...
        bROC = false;
        stResp_States = PVH_getResp_States_Feed1(strFn_States,matResp_C,AvgFrm);
